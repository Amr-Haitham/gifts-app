...
class GetPledgeStatusForGiftUseCase {
  ...
  GetPledgeStatusForGiftUseCase(this._eventsSink, this._pledgesSink, this._giftsSink, this._appUserSink);
  ...
  Future<PledgeStatusData> execute(String giftId, String eventId) async {
    try {
      ...
      if (pledge != null) {
        pledgeStatus = pledge.isFulfilled ? PledgeStatus.done : PledgeStatus.pledged;
      }
...
class GetPledgeStatusForGiftCubit extends Cubit<GetPledgeStatusForGiftState> {
  ...
  GetPledgeStatusForGiftCubit(this._getPledgeStatusForGiftUseCase) : super(GetPledgeStatusForGiftInitial());

  void getPledgeStatusForGift({required String giftId, required String eventId}) async {
    emit(GetPledgeStatusForGiftLoading());
    try {
      PledgeStatusData statusData = await _getPledgeStatusForGiftUseCase.execute(giftId, eventId);
...
class GetPledgeStatusForGiftSuccess extends GetPledgeStatusForGiftState {
  ...
  GetPledgeStatusForGiftSuccess({required this.pledgeStatus, required this.giftOwnerID});
}
